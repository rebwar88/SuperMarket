...
<head>
    ...
    <title>داشبۆردی سەرەکیی سوپەرمارکێت</title>
...
<body class="bg-slate-900 text-slate-100 min-h-screen flex flex-col">
    ...
    <header class="bg-slate-950/90 border-b border-slate-800 px-6 py-3.5 flex items-center justify-between sticky top-0 z-40 backdrop-blur">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="font-extrabold text-base tracking-tight text-white">سیستەمی بەڕێوەبردنی سوپەرمارکێت</h1>
    ...
    <main class="flex-1 p-6 max-w-7xl w-full mx-auto space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div class="bg-slate-800/80 border border-slate-700/80 rounded-2xl p-5">
                ...
                <div class="text-2xl font-black text-white mt-1.5 font-mono">{{ number_format($totalSales, 0) }} <span class="text-xs text-slate-400 font-sans">د.ع</span></div>
                <div class="text-xs text-slate-400 mt-2"><span class="text-emerald-400 font-bold font-mono">{{ $totalOrdersCount }}</span> پسوولەی بڕدراو</div>
            </div>
            ...
            <div class="bg-slate-800/80 border border-emerald-500/30 rounded-2xl p-5 relative overflow-hidden">
                ...
                <div class="text-2xl font-black text-emerald-400 mt-1.5 font-mono">{{ number_format($netProfit, 0) }} <span class="text-xs text-slate-400 font-sans">د.ع</span></div>
                <div class="text-xs text-slate-400 mt-2">تێچووی خەرجی: <span class="text-rose-400 font-mono font-bold">{{ number_format($todayExpenses, 0) }}</span> د.ع</div>
            </div>
            ...
            <div class="bg-slate-800/80 border border-amber-500/30 rounded-2xl p-5 relative overflow-hidden">
                ...
                <div class="text-2xl font-black text-amber-400 mt-1.5 font-mono">{{ number_format($customerDebt, 0) }} <span class="text-xs text-slate-400 font-sans">د.ع</span></div>
            ...
            <div class="bg-slate-800/80 border border-rose-500/30 rounded-2xl p-5 relative overflow-hidden">
                ...
                <div class="text-2xl font-black text-rose-400 mt-1.5 font-mono">{{ number_format($supplierDebt, 0) }} <span class="text-xs text-slate-400 font-sans">د.ع</span></div>
                    ...
                    <table class="w-full text-right text-xs">
                        <thead class="bg-slate-900/60 text-slate-400 font-bold border-b border-slate-700/50">
                            <tr>
                                ...
                                <th class="p-3 text-left">کۆی گشتی (د.ع)</th>
                        ...
                        <tbody class="divide-y divide-slate-700/40 font-medium">
                            @forelse ($recentOrders as $order)
                                <tr class="hover:bg-slate-700/30 transition">
                                    <td class="p-3 font-mono font-bold text-emerald-400">{{ $order->invoice_number }}</td>
                                    ...
                                    <td class="p-3">
                                        <span class="bg-slate-700 text-slate-300 px-2 py-0.5 rounded text-[11px] font-semibold">
                                            {{ $order->payment_method === 'cash' ? 'کاش' : ($order->payment_method === 'debt' ? 'قەرز' : 'کارت') }}
                                        </span>
                                    </td>
                        ...
                        @forelse ($shifts as $shift)
                            <div class="p-2.5 rounded-xl bg-slate-900/60 border border-slate-700/50 flex items-center justify-between text-xs">
                                <div>
                                    ...
                                    <span class="text-[11px] text-slate-400 font-mono">{{ $shift->opened_at ? $shift->opened_at->format('m/d H:i') : '' }}</span>
                                </div>
                                <div class="flex items-center gap-2">
                    ...
                    <div class="space-y-3">
                        @forelse ($topProducts as $top)
                            <div class="flex items-center justify-between text-xs">
                                ...
                                <span class="font-mono font-bold text-emerald-400">{{ number_format((float) $top->total_sales, 0) }} د.ع</span>
                            </div>
                        @empty
    ...
    <!-- سکریپتی چاودێری ڕاستەوخۆی ئاگادارییەکان (Live Notification Polling & Toast) -->
    <script>
        let lastNotifId = 0;
        ...
        function pollNotifications() {
            fetch("{{ route('admin.notifications.poll') }}?last_id=" + lastNotifId)
                ...
                .then(data => {
                    ...
                    // ئەگەر ئاگاداری نوێ هەبێت و یەکەم کردنەوە نەبێت، Toast دەرکەوێت
